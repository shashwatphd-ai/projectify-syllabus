...
- `course_profiles` - Parsed syllabus data
  - Fields: title, level, outcomes, schedule, artifacts, owner_id, location_city, location_state, location_zip, search_location
...
- `project_forms` - Structured project form data (6-form system)
  ...
  - Relationships: project_id → projects (1:1)
- `project_metadata` - AI-generated project analysis and scoring
  - Fields: project_id, market_alignment_score, estimated_roi, stakeholder_insights, lo_alignment_detail, pricing_breakdown
...
- `company_filter_cache` - Performance optimization (7-day TTL)
  - Purpose: Cache Apollo API results to reduce API credits
- `dashboard_analytics` - User behavior tracking
  - Fields: user_id, event_type, event_data, timestamp
  - Purpose: Analytics and usage monitoring
...
- **Output:** Updated queue items (status: processing → completed/failed)
